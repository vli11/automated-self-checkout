...
public:
    ...
    // Geti Yolox Ensemble postprocess
...
class GetiYoloXEnsemble : public ObjectDetectionInterface {
public:
        ...
        std::vector<DetectedResult> &detectedResults)
    {
        ...
        {
            printf("Unknown Geti detection and/or efficientnet-b0 model.\n");
            return;
        }
...
void run_stream(std::string mediaPath, GstElement* pipeline, GstElement* appsink, ObjectDetectionInterface* objDet)
{
    ...
    while (!shutdown_request) {
        ...
        if (_render) {
            ...
            if (dynamic_cast<const GetiYoloXEnsemble*>(objDet) != nullptr)
	        {
                metricStartTime = std::chrono::high_resolution_clock::now();
        ...
        outputId = 2;
        OVMS_InferenceResponseGetOutput(response, outputId, &outputName3, &datatype3, &shape3, &dimCount3, &voutputData3, &bytesize3, &bufferType3, &deviceId3);
        // std::cout << "------------>" << tid << " : " << "DeviceID " << deviceId1
