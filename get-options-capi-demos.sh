...
show_help() {
	echo " WIP (STREAM_DENSITY...and only RTSP inputs are implemented)
         usage: ./docker-run.sh --platform gpu|cpu|xpu --inputsrc RS_SERIAL_NUMBER|CAMERA_RTSP_URL|file:video.mp4|/dev/video0 [--realsense_enabled] --pipeline face_detection|people_detection_retail|people_detection_yolov5s|object_and_text_detection
...
while :; do
    ...
    --pipeline)
	    if [ "$2" ]; then
            ...
            error 'ERROR: "--pipeline" requires an argument face_detection|people_detection_retail|people_detection_yolov5s|object_and_text_detection'
	    fi
	    ;;
    -?*)
